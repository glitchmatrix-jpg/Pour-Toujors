...
class SimpleTodayScreen extends StatefulWidget {
  const SimpleTodayScreen({
    super.key,
  ...
  @override
  State<SimpleTodayScreen> createState() => _SimpleTodayScreenState();
}

class _SimpleTodayScreenState extends State<SimpleTodayScreen> {
  ...
  @override
  void didUpdateWidget(covariant SimpleTodayScreen oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.weatherFutures != widget.weatherFutures) {
      _bindData();
    }
  }
  ...
  Widget build(BuildContext context) {
    final awake = widget.snapshots.values
        .where((value) => value.kind != AvailabilityKind.asleep)
        .length;
    final free = widget.snapshots.values
        .where((value) => value.kind == AvailabilityKind.likelyFree)
        .length;
    ...
    return RefreshIndicator(
      ...
      child: FutureBuilder<Map<String, WeatherBundle?>>(
        ...
        builder: (context, weatherSnapshot) {
          final weather =
              weatherSnapshot.data ?? const <String, WeatherBundle?>{};
                ...
                sliver: SliverList.list(
                  children: [
                    ...
                    FutureBuilder<List<FamilyWeatherAlert>>(
                      ...
                      builder: (context, alertSnapshot) => _StatusSummary(
                        awake: awake,
                        free: free,
                        daylight: daylight,
                        advisories: _advisoryCount(
                          weather,
                          alertSnapshot.data ?? const [],
                        ),
                      ),
                    ),
                    const SizedBox(height: 26),
                    const PtSectionHeader('Your four cities'),
                    const SizedBox(height: 10),
                    if (weatherSnapshot.connectionState ==
                            ConnectionState.waiting &&
                        weather.isEmpty)
                      const PtLoadingSkeleton(height: 430)
                    else
                      _CityGrid(
                    ...
                    Text(
                      'A simple view of sleep, busy, uncertain, and likely-free routine windows. These are estimates, not live activity.',
                      style: TextStyle(color: context.pt.secondaryText),
                    ),
                    const SizedBox(height: 12),
                    _AvailabilityGrid(viewer: widget.viewer),
                    const SizedBox(height: 26),
                    const PtSectionHeader('Important today'),
                    const SizedBox(height: 10),
                    FutureBuilder<List<FamilyWeatherAlert>>(
                      future: _officialAlerts,
                      builder: (context, alertSnapshot) => _ImportantToday(
                        weather: weather,
                        officialAlerts: alertSnapshot.data ?? const [],
...
class _StatusSummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final items = [
      ...
      (Icons.light_mode_outlined, '$daylight', 'cities in daylight'),
      (Icons.cake_outlined, '${birthday.$2}', birthday.$1),
      (
        advisories == 0
    ...
    return Container(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          final columns = constraints.maxWidth >= 620 ? 5 : 2;
          final width =
                ...
                SizedBox(
                  ...
                  child: Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Icon(item.$1, size: 19, color: context.pt.accent),
                        const SizedBox(height: 10),
                        Text(
                          item.$2,
...
class _AvailabilityGrid extends StatelessWidget {
  const _AvailabilityGrid({required this.viewer});
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 8),
          for (final row in <(String, List<FamilyMember>)>[
            (viewer.name, [viewer]),
            for (final city in cities) (city.name, _members(city.id)),
          ]) ...[
            _AvailabilityRow(label: row.$1, members: row.$2, start: start),
            const SizedBox(height: 7),
          ],
          ...
          Wrap(
            ...
            runSpacing: 7,
            children: [
              _Legend(color: context.pt.asleep, label: 'Asleep'),
              _Legend(color: context.pt.working, label: 'Busy'),
              _Legend(color: context.pt.uncertain, label: 'Uncertain'),
              _Legend(color: context.pt.success, label: 'Likely free'),
            ],
          ),
        ],
      ),
...
class _AvailabilityBlock extends StatelessWidget {
  const _AvailabilityBlock({required this.instant, required this.members});
  ...
  Widget build(BuildContext context) {
    ...
    return Tooltip(
      message:
          '${DateFormat('EEE h a').format(instant.toLocal())}: ${_kindLabel(kind)}',
      child: Semantics(
        label:
            '${DateFormat('EEEE h a').format(instant.toLocal())}, ${_kindLabel(kind)}',
        child: Container(
          decoration: BoxDecoration(
            color: color.withValues(alpha: .72),
            borderRadius: BorderRadius.circular(6),
            border: instant.difference(DateTime.now().toUtc()).abs() <
...
class _ImportantToday extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (final entry in weather.entries) {
      final bundle = entry.value;
      if (bundle != null) alerts.addAll(alertService.derive(entry.key, bundle));
    }
    final birthday = _nextBirthday();
      ...
      child: Column(
        children: [
            ...
            for (final alert in alerts.take(2))
              ListTile(
                ...
                leading: CircleAvatar(
                  backgroundColor: context.pt.warning.withValues(alpha: .14),
                  child: Icon(
                    Icons.warning_amber_rounded,
          ...
          ListTile(
            ...
            leading: CircleAvatar(
              backgroundColor: context.pt.birthday.withValues(alpha: .14),
              child: Icon(Icons.cake_outlined, color: context.pt.birthday),
            ),
            title: Text('${birthday.$1} · next birthday'),
            subtitle: Text(
              birthday.$2 == 0 ? 'Today' : 'In ${birthday.$2} days',
            ),
            trailing: const Icon(Icons.chevron_right_rounded),
          ),
        ],
...
AvailabilityKind _dominantKind(List<AvailabilityKind> kinds) {
...
(String, int) _nextBirthday() {
  ...
  final values = [...familyBirthdays]
    ..sort((a, b) => a.nextOccurrence(now).compareTo(b.nextOccurrence(now)));
  final next = values.first.nextOccurrence(now);
  return (
    values.first.name,
