...
class NationalHoliday {
  const NationalHoliday({required this.date, required this.name, required this.countryCode});
  final DateTime date;
  final String name;
...
class HolidayService {
  ...
  Future<List<NationalHoliday>> fetchUpcoming(String cityId, {int limit = 3}) async {
    ...
    for (final year in [now.year, now.year + 1]) {
      final response = await http
          .get(Uri.https('date.nager.at', '/api/v3/PublicHolidays/$year/$code'))
